...
export class HereyaDevEnvAwsStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const setupLines = [
      ...
      '',
      '# Enable + start crond. AL2023 ships cronie installed but the service',
      '# is NOT auto-started, so /etc/cron.d/* entries (hereya-update and the',
      '# idle-stop activity tracker below) sit there unused unless we enable',
      '# the daemon here explicitly. Without this, devenvs run forever — the',
      '# activity cron never fires, neither does the metric, neither does the',
      '# self-stop, and you pay for an idle t3.medium until you notice.',
